...
覆盖至少一个当前缺口，否则给可读中文错误——手动通道解决"规则太严"，
不解决"下错片"。
...
async def grab_manual(
    ...
    *,
    ...
    identity = MediaIdentity(
        ...
        season_numbers=tuple(sorted({s for s, _ in open_wanted})),
    )
    ...
    if not covered:
        gap_text = "、".join(
            "正片" if s == 0 and e == 0 else f"S{s:02d}E{e:02d}"
            for s, e in sorted(open_wanted)[:8]
        )
        raise BadRequestException(
            f"该种子识别为{_match_text(match)}，但当前缺口是 {gap_text}"
            f"{'…' if len(open_wanted) > 8 else ''}——没有可满足的追踪项"
        )
    ...
    if not done:
        raise ConflictException("这些缺口刚被自动管线抢先投递，无需重复操作——详见活动记录")
    return covered
