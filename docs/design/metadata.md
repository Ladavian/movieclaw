...
**档位变更的存量升级**：`media_metadata.asset_profile` 记录下载时用的档位
组合，与当前配置不符即在下次刷新时重下——否则调高画质只对新条目生效，
存量库永远停在旧档位。升级**不受选图锁限制**：锁保护的是"用哪张图"，
不是"用什么分辨率"。
...
**铁律**（沿用现有 NFO 写出原则并扩展到图片）：

- 目标文件已存在 → 跳过，**绝不覆盖**（存量刮削成果可能更丰富）；
  唯一例外：**单条目 force 刷新时图片覆盖写**——用户点强制刷新的语义
  就是"以 TMDB 最新为准"（Emby「替换现有图片」同款）；NFO 不跟随覆盖
  （TMM/Emby 的富 NFO 是用户刮削成果，维持只增规则）；
- **绝不删除**媒体目录中的任何文件；
- 写失败（只读挂载/权限）只告警不阻断刮削事务。

NFO 从现有「最小身份 NFO」升级为**完整 NFO**（plot/rating/runtime/genre/
actor 带 thumb），`library_nfo.py` 新增 `write_full_nfo`，数据源即
`media_metadata`。默认开的理由：只增不覆盖使其无破坏性，且「反哺播放器生态」
正是用户诉求；不想污染目录的用户按库关闭。
...
- **海报**：本地化语言优先（中文海报比英文原版更合预期）→ 宽度 ≥500 → 同上加权分；
